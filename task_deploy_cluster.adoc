...
. Cliquez image:icon_question_mark.gif["Point d'interrogation"] en haut de la page.
...
Déploiement de l'installation et des identifiants du compte:: Vous devez installer l'utilitaire d'administration de déploiement et effectuer la configuration initiale. Voir link:task_install_deploy.html["Installez ONTAP Select Deploy"] pour plus d'informations. Vous devez disposer du mot de passe du compte administrateur de déploiement configuré dans le cadre du processus d'installation.
...
.Avant de commencer
Reportez-vous  à la section link:task_deploy_cluster.html#before-you-begin["Avant de commencer"] pour préparer le déploiement du cluster. L'administration de déploiement doit être installée et configurée initialement (mot de passe, AutoSupport et vCenter).
...
. Si la page de lancement du cluster *Getting Started* ne s'affiche pas, cliquez image:icon_question_mark.gif["Point d'interrogation"]en haut de la page et cliquez sur *Getting Started*.
